...
Approximation für sehr großes n: $\binom{n}{k}p^k(1-p)^{n-k}=Binomial(n,p)(\set{k})$
\subsection{Poisson}
...
$f(x):=\delta e^{-\delta x} \mathbf{1}_{(0,\infty)(x)}, \delta >0$
$\mathbb{E}(X)=\frac{1}{\delta}$
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{Rechenregelncovarianz}
...
\section{Statistik}
Maximum-Likehood-Schätzer Standartmodell
\end{document}
